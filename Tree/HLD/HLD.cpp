...
// Usage:
// order: the vertex ordering while visiting in HLD
// orderPos[i]: position of vertex i in HLD
// order[orderPos[u] - 1]: origin vertex of orderPos[u]
// decompose(u, v): decompose path u-v to O(log) intervals in `order` and orderPos (down to top)
// if use IT/BIT, build with array orderPos 
...
struct HLD {
    ...
    // decompose path u-par into log intervals in order (down to top)
