...
// Time: O(nlogn)
// If answer does not modulo MOD:
// set N = (1 << 15) (line 56) and delete all MOD parts (81 - 86)
// MOD be prime and <= 10^9 + 7
// with x^a * x^b, time = max(a, b)
// use maxDeg if find coef of x^k (k <= 10^6) and degree large (~ 10^18)
...
vector<long long> multiply(vector<long long>& v, vector<long long>& w, int maxDeg = -1) {
    int n = 2;
    ...
    if (maxDeg != -1) sz = min(sz, maxDeg + 1);
